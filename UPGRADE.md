...
### 4. Cargo flows: crude, products, LNG (LNG BUILT as `commodity-lng`; crude and products PLANNED)

| Free now | What it lacks | Paid upgrades | What the globe gains |
| --- | --- | --- | --- |
| **LNG (built):** GEM LNG terminals (Sept 2025 release, operating and under construction), EIA train table (quarterly), DOE cargo-level US exports (monthly, ~2-month lag), the GIIGNL annual import matrix re-typed from the public PDF, searoute-ts shortest sea routes with a Red Sea-closed variant | Live LNG carrier positions (no keyless AIS exists; an IMO allowlist from the DOE tanker column is needed even with AIS), cargo-level non-US trade (GIIGNL is annual and country-to-country), observed routes rather than modelled shortest paths, GEM's operator column and GIS pack (behind the email form), monthly widths without a rebuild (an EIA API key, free, is milestone 7) | **AISStream** key (free; carriers as a chip on the vessels row), **Kpler** (LNG cargoes terminal to terminal), **Vortexa** (LNG flows and freight), **Spark Commodities** (LNG freight and netbacks), **ICIS LNG Edge** (cargo tracking, netbacks), **GIIGNL members' workbook** (the matrix as data, quarterly), **GEM GIS download** (routes and terminal polygons) | Arcs that follow observed tracks with laden and ballast split, monthly non-US pairs, carriers drawn on the arcs, Panama and Suez transits as events rather than a modelled via list |
| EIA crude imports by origin (monthly, US only), EIA natural gas trade routes, UN Comtrade preview (annual, codes only), JODI monthly balances, ENTSOG EU pipeline flows | Vessel-level cargoes, floating storage, grade and quality, non-US destinations, weekly cadence | **Kpler** (flows by grade and route), **Vortexa** (flows, floating storage, freight), **S&P Global Commodities at Sea**, **Argus Media** | Flow arcs by grade with weekly widths, floating storage highlighted off Singapore and the Gulf |
